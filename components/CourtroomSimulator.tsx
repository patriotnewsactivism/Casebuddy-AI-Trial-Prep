...
import {
  ...
  RotateCcw, ChevronRight, Loader2, Star, AlertTriangle,
  CheckCircle, MessageSquare, Users, Scale, Brain,
  TrendingUp, Award, Zap, BookOpen, X
} from 'lucide-react';
...
async function getCourtResponse(
  ...
  const personas: Record<Speaker, string> = {
    judge: `You are a federal district court judge — strict, fair, procedurally rigorous. You rule on objections instantly, cut off rambling, and demand precision. Keep responses under 3 sentences unless ruling on a major motion.`,
    opposing: `You are aggressive opposing counsel — sharp, tactical, always looking for weaknesses. Object frequently, challenge credibility, and exploit procedural missteps. Keep responses concise and combative.`,
    ...
    coach: `You are a master trial attorney coaching an attorney AFTER their statement. Give sharp, specific feedback in 1-2 sentences. Focus on what they should do NEXT and any immediate tactical improvement.`,
  };
...
Respond in JSON: {"text": "<your response>", "coaching": "<1 tactical tip for the attorney right now>"}`;
  ...
  try {
    ...
    return { text: raw.substring(0, 200), coaching: '' };
  }
}

// ── Gemini — final performance score ─────────────────────────────────────────
...
Score in VALID JSON only:
{
  "overall": <0-100>,
  "categories": [
    {"label": "Argument Quality", "score": <0-100>, "feedback": "<specific feedback>"},
    {"label": "Objection Handling", "score": <0-100>, "feedback": "<specific feedback>"},
    {"label": "Witness Control", "score": <0-100>, "feedback": "<specific feedback>"},
    {"label": "Persuasiveness", "score": <0-100>, "feedback": "<specific feedback>"},
    {"label": "Procedural Precision", "score": <0-100>, "feedback": "<specific feedback>"}
  ],
  "strengths": ["<strength 1>", "<strength 2>"],
  "weaknesses": ["<weakness 1>", "<weakness 2>"],
  "verdict": "<Likely win/loss if this were a real trial and why in 1 sentence>",
  "recommendation": "<The single most important thing to work on>"
}`;
...
const CourtroomSimulator: React.FC = () => {
  ...
  const [opponentName, setOpponentName] = useState(activeCase?.opposingCounsel || 'counsel for the defense');
  ...
  const addMsg = (speaker: Speaker | 'attorney', text: string, coaching?: string): Message => {
  ...
  const startTrial = useCallback(async () => {
    ...
    const openings: Record<TrialMode, string> = {
      ...
      full: `Court is now in session, the matter of ${activeCase?.title || 'the case before us'}. ${judgeName} presiding. Are counsel ready? You may proceed with opening statements.`,
    };
    ...
    setAiSpeaking(null);
    setLoading(false);
  }, [mode, muted, judgeName, opponentName, activeCase]);

  // ── Attorney speaks ──────────────────────────────────────────────────────
  const handleAttorneyStatement = useCallback(async (text?: string) => {
    const statement = text || input.trim();
    ...
    try {
      ...
      // Opposing objects ~30% of the time in full/examination mode
      if ((mode === 'full' || mode === 'examination') && Math.random() < 0.3 && primary !== 'opposing') {
        const { text: objection } = await getCourtResponse('opposing', messages, statement, caseContext, mode);
        addMsg('opposing', objection);
        setAiSpeaking('opposing');
        await speak(objection, 'opposing', muted);
        setAiSpeaking(null);
      }
    } catch (e) {
      toast.error('Response failed — check your API connection');
    } finally {
      setLoading(false);
    }
  }, [input, loading, messages, mode, muted, showCoach, caseContext]);
  ...
  const startListening = useCallback(() => {
    ...
    r.onresult = (e: any) => {
      ...
      if (t) handleAttorneyStatement(t);
    };
  ...
  }, [messages, caseContext, mode, muted]);
        ...
        <div className="max-w-2xl w-full">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 mb-4">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <label htmlFor="coach" className="text-sm text-slate-300">Show real-time AI coaching tips after each response</label>
            </div>
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col">
        ...
        <div className="border-b border-slate-800 px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            {aiSpeaking && (
              <span className={`text-xs px-2 py-0.5 rounded-full border ${speakerColors[aiSpeaking]} ${speakerLabel[aiSpeaking]}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setPhase('setup')} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors">
              <X size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="border-t border-slate-800 p-4">
          <div className="flex items-center gap-3 max-w-3xl mx-auto">
            <button
              ...
              disabled={loading}
              className={`w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${listening ? 'bg-red-500 shadow-lg shadow-red-500/30 scale-110' : 'bg-slate-700 hover:bg-emerald-600'} text-white disabled:opacity-50`}>
              {listening ? <MicOff size={18} /> : <Mic size={18} />}
            </button>
            <input
              value={input} onChange={e => setInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleAttorneyStatement()}
              disabled={loading || listening}
              placeholder={listening ? 'Listening...' : 'State your argument, objection, or question...'}
            ...
            />
            <button onClick={() => handleAttorneyStatement()} disabled={!input.trim() || loading}
              className="w-11 h-11 rounded-full bg-emerald-600 hover:bg-emerald-500 flex items-center justify-center flex-shrink-0 text-white disabled:opacity-50 transition-colors">
              {loading ? <Loader2 size={18} className="animate-spin" /> : <ChevronRight size={18} />}
            </button>
          </div>
          <p className="text-center text-slate-600 text-xs mt-2">Hold mic to speak • Enter to send • End session to get your performance score</p>
        </div>
      </div>
    );
  }
