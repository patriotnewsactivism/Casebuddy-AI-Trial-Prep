...
import { openPrintExport, escapeHtml } from '../lib/printExport';
...
<div class="meta">
  <strong>Witness:</strong> ${escapeHtml(form.name)} · <strong>Type:</strong> ${escapeHtml(form.witness_type)} · <strong>Side:</strong> ${form.side === 'ours' ? 'Our Witness' : 'Opposing'}
  ${form.occupation ? ` · <strong>Occupation:</strong> ${escapeHtml(form.occupation)}` : ''}
  <br>Prepared by Rex · ${new Date().toLocaleDateString()}
</div>
<div class="box"><strong>Strategic Overview:</strong><br>${escapeHtml(result.ai_prep_notes || '')}</div>
<p><strong>Credibility Assessment:</strong> ${escapeHtml(result.credibility_assessment || 'N/A')}</p>
${result.opening_gambit ? `<h2>🎯 Opening Gambit</h2><p>${escapeHtml(result.opening_gambit)}</p>` : ''}
<h2>✅ Direct Examination Questions (${result.direct_questions?.length || 0})</h2>
<ol>${(result.direct_questions || []).map((q: string) => `<li>${escapeHtml(q)}</li>`).join('')}</ol>
<h2>⚡ Cross Examination Questions (${result.cross_questions?.length || 0})</h2>
<ol>${(result.cross_questions || []).map((q: string) => `<li>${escapeHtml(q)}</li>`).join('')}</ol>
<h2>🎯 Vulnerabilities to Exploit</h2>
<ul>${(result.vulnerabilities || []).map((v: string) => `<li>${escapeHtml(v)}</li>`).join('')}</ul>
